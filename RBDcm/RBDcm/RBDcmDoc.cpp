...
void CRBDcmDoc::OnImginfodis()
{
	...
	((CMainFrame*)AfxGetMainWnd())->m_pLUView->Invalidate(FALSE);
}
...
DWORD WINAPI CRBDcmDoc::PlaySerileProc(LPVOID lpParameter)
{
	...
	while(num > index)
	{
		pDoc->m_showImage = pDoc->m_matimg.NextMatImg().masking;
		pDoc->ShowImg();
		Sleep(1000);
		//SendMessage();
		//pRDBView->ReDraw();
...
{
	m_srcImage = m_matimg.LastMatImg().masking.clone();
	m_showImage = m_srcImage.clone();
	ShowImg();
	//COprtView *pOpt = (COprtView*)(((CMainFrame*)AfxGetMainWnd())->m_splt.GetPane(0, 0));
	((CMainFrame*)AfxGetMainWnd())->m_pOpt->SetCurrent(m_matimg.GetCurrentNum());
...
void CRBDcmDoc::Next(void)
{
	...
	m_srcImage = m_matimg.NextMatImg().masking.clone();
	//m_srcImage = rt.clone();
	m_showImage = m_srcImage.clone();
	ShowImg();
